  ...
  public:
    static Color getLightingSimple(const Shape &object, const Vector3 &point, const Vector3 &normal, const Vector3 &view, const vector<Light*> &lights, const vector<Shape*> &objects) {
      ...
      for(int i = 0; i < lights.size(); i++) {
        ...
        if (!isInShadow)
          rayColor +=  _getLighting(object, point, normal, view, lights[i]);
        //else
        //  rayColor += Color(0);
      }
    ...
    static Color getLighting(const Shape &object, const Vector3 &point, const Vector3 &normal, const Vector3 &view, const vector<Light*> &lights, const vector<Shape*> &objects) {
      ...
      for(int i = 0; i < lights.size(); i++) {
        ...
        rayColor += _getLighting(object, point, normal, view, lights[i]) * (1.0 - shadowFactor);
      }
    ...
    static Color _getLighting(const Shape &object, const Vector3 &point, const Vector3 &normal, const Vector3 &view, const Light *light) {
      Color rayColor;
...
// Binding code
EMSCRIPTEN_BINDINGS(lighting) {
  class_<Lighting>("Lighting")   
    .class_function("getLighting", &Lighting::getLighting)
    ;
}